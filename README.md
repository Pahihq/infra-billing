...
> `NODE_ENV` по умолчанию `production` (задавать не нужно). Базовая валюта, источник курсов
> (`cbr`/`manual`), интервал автосинка, дни предупреждения о списании и Telegram **настраиваются в
> панели** (хранятся в БД, токен Telegram шифруется) — не через env.
...
## Команды (Makefile) и релиз

`make help` — все цели. Основное: `install`, `dev`, `build`, `typecheck`, `lint`, `check`, `test`,
`db-up`/`db-down`, `migrate`, `generate`, `studio`, `docker-build`/`docker-up`/`docker-down`.

Релиз: `make bump-patch|minor|major` (бампит версию всех пакетов) → `make tag-release` (подписанный
git-тег). На пуш тега CI (`.github/workflows/build-and-push.yml`) собирает multi-arch образ, пушит
в GHCR (`:<версия>` + `:latest`), создаёт GitHub-релиз и шлёт уведомление в Telegram.

## Тесты

```bash
make test     # сборка + юнит-тесты (node --test): нормализация стоимости, конвертация валют,
              # маппинг ответов коннекторов (Timeweb/Hetzner/HostBill/BILLmanager/Selectel/4VPS)
```
